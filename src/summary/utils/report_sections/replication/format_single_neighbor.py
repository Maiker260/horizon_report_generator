...
def format_single_neighbor(naming_context, neighbor, naming_context_max_width):
    ...
    if is_replication_successful(neighbor):
        content.append(f"{indent}{naming_context + ':':<{naming_context_max_width + 1}}  {status} ({last_attempt})")

        return content

    # Failed replication
    content.append(f"{indent}{naming_context}: {status}")
