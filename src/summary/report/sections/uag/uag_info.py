...
def uag_info(data, component, letter):
    ...
    for section, fields in sections.items():
        ...
        if section == "URLs":
            normalized_fields = [normalize_uag_titles(field) for field in fields]

            max_width = max(len(name + ":") for name in normalized_fields)

            # Groups of related URLs separated by an empty line.
            url_groups = [
                ["proxyDestinationUrl", "proxyDestinationUrlThumbprints"],
                ["pcoipExternalUrl"],
                ["blastExternalUrl", "blastUrls"],
                ["tunnelExternalUrl", "tunnelUrls"],
            ]

            field_display_names = dict(zip(fields, normalized_fields))

            for group_index, group in enumerate(url_groups):
                for field in group:
                ...
                # Empty line between URL groups, but not after the last one.
                if group_index < len(url_groups) - 1:
                    content.append("")
